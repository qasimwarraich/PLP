...
void fizzbuzz(int m, int n, int fizz, int buzz)
{
    ...
    for (; m <= n; m++) {
        ...
        } else if ( m % buzz == 0) { std::cout << "Buzz "; }
...
int main()
{
    ...
    while (!exit) {
        ...
        if (user_input == "quit" || user_input == "q"){
            ...
            try {
            if (n != 4) throw ">> Please enter exactly 4 Integers.\n"; 
            if (input[0] >= input [1]) throw ">> Integer m must be greater than n.\n";
            if (input[2] == input [3]) throw ">> Integers x and y must not be the equal.\n"; 
            for (int i = 0; i < n; i++) if (input[i] < 1) throw ">> Integers must be positive.\n";
